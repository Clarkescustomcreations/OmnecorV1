...
export const aiProviderRouter = router({
  ...
  chatStream: publicProcedure
    ...
    .subscription(({ input }) => {
      return observable<{ delta: string; done: boolean; totalTokens?: number }>((emit) => {
        const svc = AiProviderService.getInstance();
        (async () => {
          for await (const chunk of svc.streamChat(input, input.messages, input.systemPrompt)) {
            emit.next(chunk);
            if (chunk.done) { emit.complete(); break; }
          }
